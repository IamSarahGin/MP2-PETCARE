...
const AppointmentList = () => {
  ...
  const fetchPendingBookings = async () => {
    try {
      const response = await axios.get('http://localhost:3001.com/api/bookings/pending', {
  ...
  const updateBookingStatus = async (bookingId, newStatus) => {
    try {
      await axios.post(`http://localhost:3001.com1/api/bookings/update-status/${bookingId}/${newStatus}`, {}, {
        withCredentials: true,
        headers: {
          'Content-Type': 'application/json'
        }
