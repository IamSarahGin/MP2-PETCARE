...
const BookingComponent = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const serviceResponse = await axios.get('http://localhost:3001.com/serviceTypes');
                setServiceTypes(serviceResponse.data);

                const petResponse = await axios.get('http://localhost:3001.com/petTypes');
                setPetTypes(petResponse.data);

                const userResponse = await axios.get('http://localhost:3001.com/api/currentUserLoginId');
    ...
    const checkExistingBookings = async () => {
        try {
            const response = await axios.get(`http://localhost:3001.com/existingBookings?userId=${userId}&date=${date}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const bookingResponse = await axios.post('http://localhost:3001.com/bookings', {
                petName,
                petId,
                breed,
                age,
            ...
            await axios.post('http://localhost:3001.com/bookTimeSlot', {
                date,
                time: time.split('-')[0],
                userId,
                email: userEmail // Use the email obtained from the backend response
