...
const BookingComponent = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const serviceResponse = await axios.get('https://mp2-petcare.onrender.com/serviceTypes');
                setServiceTypes(serviceResponse.data);

                const petResponse = await axios.get('https://mp2-petcare.onrender.com/petTypes');
                setPetTypes(petResponse.data);

                const userResponse = await axios.get('https://mp2-petcare.onrender.com/api/currentUserLoginId');
    ...
    const checkExistingBookings = async () => {
        try {
            const response = await axios.get(`https://mp2-petcare.onrender.com/existingBookings?userId=${userId}&date=${date}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const bookingResponse = await axios.post('https://mp2-petcare.onrender.com/bookings', {
                petName,
                petId,
                breed,
                age,
            ...
            await axios.post('hhttps://mp2-petcare.onrender.com/bookTimeSlot', {
                date,
                time: time.split('-')[0],
                userId,
                email: userEmail // Use the email obtained from the backend response
