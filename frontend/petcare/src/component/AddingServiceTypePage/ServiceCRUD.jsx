...
const ServiceCRUD = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get('http://localhost:3001.com/api/user/profile', { withCredentials: true });
  ...
  const fetchServices = async () => {
    try {
      const response = await axios.get('http://localhost:3001.com/serviceTypes');
  ...
  const handleCreateService = async () => {
    try {
      await axios.post('http://localhost:3001.com/services', { serviceType });
      setServiceType('');
      fetchServices();
    } catch (error) {
  ...
  const handleUpdateService = async () => {
    try {
      await axios.put(`http://localhost:3001.com/service/${updateServiceId}`, { serviceType: updateServiceType });
  ...
  const handleDeleteService = async (serviceId) => {
    try {
      await axios.delete(`http://localhost:3001.com/service/${serviceId}`);
      fetchServices();
    } catch (error) {
