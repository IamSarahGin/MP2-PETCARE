...
const Login = () => {
  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        const response = await axios.get('http://localhost:3001.com/loginpageandregisterpage');
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post('http://localhost:3001.com/login', values, { withCredentials: true });
