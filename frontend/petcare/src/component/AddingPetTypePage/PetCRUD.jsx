...
const PetCRUD = () => {
  ...
  const fetchPets = async () => {
    try {
      const response = await axios.get('https://mp2-petcare.onrender.com/api/pet');
  ...
  const handleCreatePet = async () => {
    try {
      await axios.post('https://mp2-petcare.onrender.com/api/pet', { petType });
      setPetType('');
      fetchPets();
    } catch (error) {
  ...
  const handleUpdatePet = async () => {
    try {
      await axios.put(`https://mp2-petcare.onrender.com/api/pet/${updatePetId}`, { petType: updatePetType });
      setUpdatePetId(null);
      setUpdatePetType('');
      fetchPets();
    } catch (error) {
  ...
  const handleDeletePet = async (petId) => {
    try {
      await axios.delete(`https://mp2-petcare.onrender.com/api/pet/${petId}`);
      fetchPets();
    } catch (error) {
