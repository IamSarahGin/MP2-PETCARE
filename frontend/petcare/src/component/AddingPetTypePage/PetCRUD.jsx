...
const PetCRUD = () => {
  ...
  const fetchPets = async () => {
    try {
      const response = await axios.get('http://localhost:3001.com/api/pet');
  ...
  const handleCreatePet = async () => {
    try {
      await axios.post('http://localhost:3001.com/api/pet', { petType });
      setPetType('');
      fetchPets();
    } catch (error) {
  ...
  const handleUpdatePet = async () => {
    try {
      await axios.put(`http://localhost:3001.com/api/pet/${updatePetId}`, { petType: updatePetType });
      setUpdatePetId(null);
      setUpdatePetType('');
      fetchPets();
    } catch (error) {
  ...
  const handleDeletePet = async (petId) => {
    try {
      await axios.delete(`http://localhost:3001.com/api/pet/${petId}`);
      fetchPets();
    } catch (error) {
